...
def main(args):
    if args.eval_dataset == "sintel":
        ...
        bonn_root = dataset_metadata.get(args.eval_dataset, {}).get(
        ...
        kitti_img_root = dataset_metadata.get(args.eval_dataset, {}).get(
            "img_path",
            f"data/long_kitti_s1/depth_selection/val_selection_cropped/image_gathered_{kitti_number}",
        )
        if "image_gathered_" in kitti_img_root:
            kitti_depth_root = kitti_img_root.replace(
                "image_gathered_", "groundtruth_depth_gathered_"
            )
        else:
            kitti_depth_root = (
                "data/long_kitti_s1/depth_selection/val_selection_cropped/"
                f"groundtruth_depth_gathered_{kitti_number}"
            )

        depth_pathes = glob.glob(f"{kitti_depth_root}/*/*.png")
        depth_pathes = sorted(depth_pathes)
        pred_pathes = glob.glob(
            f"{args.output_dir}/*/frame_*.npy"
        )  # TODO: update the path to your prediction
