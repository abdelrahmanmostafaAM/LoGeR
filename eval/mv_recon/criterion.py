...
from eval.mv_recon.utils.misc import invalid_to_zeros, invalid_to_nans
from eval.mv_recon.utils.geometry import inv, geotrf, depthmap_to_pts3d
...
def get_pred_pts3d(gt, pred, use_pose=False):
    ...
    if use_pose:
        ...
        if not torch.is_tensor(camera_pose):
            camera_pose = torch.as_tensor(camera_pose)
        camera_pose = camera_pose.to(pts3d.device, dtype=pts3d.dtype)
        if camera_pose.ndim == 3 and camera_pose.shape[-2:] == (4, 4):
            cam_matrix = camera_pose
        elif camera_pose.ndim == 2 and camera_pose.shape == (4, 4):
            cam_matrix = camera_pose.unsqueeze(0)
        pts3d = geotrf(cam_matrix, pts3d)
