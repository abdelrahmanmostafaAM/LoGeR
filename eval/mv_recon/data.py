...
from eval.mv_recon.utils.image import imread_cv2
...
class SevenScenes(BaseStereoViewDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.frame_sampling = str(frame_sampling).lower()
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            ...
            if self.max_frames is not None:
                if self.frame_sampling == "uniform" and len(img_idxs) > self.max_frames:
                    if self.max_frames <= 1:
                        img_idxs = img_idxs[:1]
                    else:
                        total = len(img_idxs)
                        # Evenly sample indices while keeping temporal order.
                        sample_ids = [
                            (i * (total - 1)) // (self.max_frames - 1)
                            for i in range(self.max_frames)
                        ]
                        img_idxs = [img_idxs[i] for i in sample_ids]
                else:
...
class Long3D(BaseStereoViewDataset):
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        if self.tuple_list is not None:
            ...
            image_dir = os.path.join(self.ROOT, scene_id, "images", "scan_images")
            if not os.path.exists(image_dir):
                # Fallback if structure is slightly different or unzipped differently
                image_dir = os.path.join(self.ROOT, scene_id, "images")
            
            if not os.path.exists(image_dir):
                 # Try finding where images are
                 for root, dirs, files in os.walk(os.path.join(self.ROOT, scene_id)):
                     if "scan_images" in dirs:
                         image_dir = os.path.join(root, "scan_images")
                         break
            
            all_files = sorted([f for f in os.listdir(image_dir) if f.endswith('.jpg') or f.endswith('.png')])
            img_idxs = all_files
            img_idxs = img_idxs[:: self.kf_every]
            if self.max_frames is not None:
                img_idxs = img_idxs[:self.max_frames]

        # Default intrinsics for 1920x1080 (approximate)
        # Assuming roughly 60 deg FOV horizontal?
        # fx = W / (2 * tan(fov/2))
        # 1920 / (2 * tan(30)) ~ 1662
        # Let's use something standard or just placeholder since we don't have GT anyway.
        # But Pi3 might use it for unprojection.
        # Let's use 1000 as a placeholder focal length.
        W, H = 1920, 1080
        fx, fy, cx, cy = 1500, 1500, W/2, H/2 
        intrinsics_ = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]], dtype=np.float32)
        ...
        while len(imgs_idxs) > 0:
            im_filename = imgs_idxs.popleft()
            
            # Direct path construction
            impath = os.path.join(image_dir, im_filename)
            
            if not os.path.exists(impath):
                print(f"Warning: Could not find {impath}")
                continue

            rgb_image = imread_cv2(impath)
            
            # Create dummy depthmap (ones to ensure validity)
            ...
            fx = 1.5 * max(W_img, H_img)
            fy = 1.5 * max(W_img, H_img)
            cx = W_img / 2.0
            cy = H_img / 2.0
            ...
            views.append(
                dict(
                    ...
                    camera_intrinsics=intrinsics,
                    dataset="long3d",
                    label=osp.join(scene_id, im_filename),
                    instance=impath,
                )
            )
