...
class BaseStereoViewDataset:
    ...
    def _crop_resize_if_necessary(
        ...
        return self._ensure_patch_alignment(image, depthmap, intrinsics)

    def _ensure_patch_alignment(self, image, depthmap, intrinsics):
        """Center-crop a tiny border so H/W are multiples of PATCH_ALIGN."""
        if PATCH_ALIGN <= 1:
            return image, depthmap, intrinsics

        width, height = image.size
        target_w = (width // PATCH_ALIGN) * PATCH_ALIGN
        target_h = (height // PATCH_ALIGN) * PATCH_ALIGN

        if target_w == width and target_h == height:
            return image, depthmap, intrinsics

        if target_w < PATCH_ALIGN or target_h < PATCH_ALIGN:
            raise ValueError(
                f"Cannot enforce patch alignment: {width}x{height} smaller than stride {PATCH_ALIGN}"
            )
        ...
        return cropping.crop_image_depthmap(image, depthmap, intrinsics, crop_bbox)
