...
def load_replica_traj(gt_file):
    ...
    for i, r in enumerate(traj_w_c):
        ...
        if np.abs(np.linalg.det(pose[:3, :3]) - 1.0) > 1e-2:
            try:
    ...
    timestamps_mat = np.array(valid_indices).astype(float)
...
def eval_metrics(pred_traj, gt_traj=None, seq="", filename="", sample_stride=1):
    ...
    if isinstance(pred_traj, (tuple, list)) and len(pred_traj) > 0:
        if not np.all(np.isfinite(pred_traj[0])):
            print(f"Warning: Prediction contains NaNs/Infs in sequence {seq}")
    ...
    if gt_traj is not None:
        ...
        if len(gt_traj.timestamps) < 2:
            print(f"Warning: Not enough associated poses for {seq}")
            return np.nan, np.nan, np.nan
    ...
    try:
        ...
        print(f"APE computation failed for {seq}: {e}")
        return np.nan, np.nan, np.nan
    ...
    try:
        ...
        print(f"RPE computation failed for {seq}: {e}")
